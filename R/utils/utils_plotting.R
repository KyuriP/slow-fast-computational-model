# ============================================================
# Plotting helpers for slow-fast simulations
# ============================================================
# Active plotting utilities for:
#   - time-series panels
#   - bifurcation overlays
#   - bifurcation diagrams
#   - optional shock-time annotations
# ============================================================
...
# ------------------------------------------------------------
# Fold helper: convert fold locations to a plotting data frame
# ------------------------------------------------------------
folds_to_df <- function(folds) {
  if (is.null(folds)) {
    return(tibble::tibble(P_fold = numeric(0), lab = character(0)))
  }
...
# ------------------------------------------------------------
# Options:
#   - optional group relabeling
#   - optional burn-in removal
#   - optional restriction to the final t_window steps
...
# ------------------------------------------------------------
# Extract shock times from a simulation table
# ------------------------------------------------------------
# Returns:
#   - distinct shock times pooled across groups, or
#   - distinct (group, t) pairs if by_group = TRUE
shock_times <- function(sim_df, burn_in = 0, by_group = FALSE) {
  ...
  if (nrow(df) == 0) {
    return(
      if (by_group) tibble::tibble(group = character(0), t = integer(0))
      else tibble::tibble(t = integer(0))
    )
  }
  
  if (by_group) {
    df %>% dplyr::distinct(group, t)
  } else {
    df %>% dplyr::distinct(t)
  }
}
...
# ------------------------------------------------------------
# Time-series panel: P(t) and m(t), with optional fold and shock lines
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Bifurcation overlay: equilibrium m*(P) with simulated trajectory points
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Standalone bifurcation diagram
# ------------------------------------------------------------
# Stable lower and upper branches are drawn separately to avoid
# visually filling the bistable region as a single slab.
